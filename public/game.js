...
// Server-based authentication system is now used
// HybridAuthSystem has been replaced with ServerAuthSystem

// Server-based authentication system is now used
// window.serverAuth is initialized in server-auth.js
...
// Send total score to server in real-time (Score = Total Score)
async function sendCoinsToServer(coinsGained) {
    const currentUser = window.gameUtils?.getCurrentUser();
    if (!currentUser || !window.localPlayer) {
        console.log('⚠️ Cannot save score - no authenticated user or localPlayer');
        return;
    }
    
    try {
        const totalScore = window.localPlayer.score || 0;
        
        // Update user stats via server API
        const updatedStats = {
            ...currentUser.stats,
...
function setupSocketListeners() {
    ...
    socket.on('gameUpdate', (data) => {
        ...
        if (localPlayer) {
            ...
            // Force update max speed display when Player Eater state changes
            if (localPlayer) {
...
function updateCamera() {
    ...
    // Use global calculateSpeedMultiplier function
        ...
        if (now - lastStatsUpdate > 1000 && localPlayer) {
            ...
            if (currentUser && window.authSystem?.currentUser && localPlayer.score > 0) {
                ...
                if (localPlayer.score > savedTotalScore) {
                    ...
                    sendCoinsToServer(localPlayer.score - savedTotalScore);
                }
            }
